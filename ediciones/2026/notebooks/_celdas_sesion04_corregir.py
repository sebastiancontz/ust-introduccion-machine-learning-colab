#!/usr/bin/env python3
"""Celdas de la PRIMERA MITAD de la sesión 4: decidir sobre lo que está mal.

Ya no construye un notebook por su cuenta. Expone `CELDAS` y sus ayudantes para que
`_build_04_preparacion.py` las empalme con las de la segunda mitad en el cuaderno único de la
sesión. Su versión como notebook SUELTO está archivada en `_sesiones-separadas/notebooks/`.
...
"""),
    md("""
# 04 · Sesión 4, parte A — corregir lo que está mal

La sesión pasada **detectamos y documentamos**. Hoy **decidimos y corregimos**.

Este es el **primero de los dos cuadernos de la sesión**: acá se decide sobre lo que está MAL en el
archivo. La **parte B** (`05-ingenieria-variables.ipynb`) transforma lo que ya está bien, parte del
archivo que este cuaderno deja corregido, y se abre después del receso. La bitácora se abre acá y se
sigue llenando allá: es un solo documento.
