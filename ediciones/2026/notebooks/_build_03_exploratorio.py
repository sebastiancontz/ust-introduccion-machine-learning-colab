...
CARGA DE LA BITÁCORA (decisión del docente, 2026-08-11): el estudiante llena TRES plantillas, no
cinco. Con seis campos cada una eran 30 casillas, contra las 10 de la clase 4, y la proporción
estaba invertida: allá se modela más de lo que se pide y acá se pedía más de lo que se modelaba.
Se descartó fusionar `descripción` con `evidencia` —la tesis de esta clase es que un hallazgo sin
evidencia no es un hallazgo, así que ese campo es load-bearing acá de un modo que en la clase 4 no
lo era— y se bajó la cantidad de plantillas. Salieron las dos que menos aportaban: la del par
monto_neto/monto_total, que el paso 3 ya había decidido en clase y solo pedía transcribir, y la de
razón social, cuyo argumento se conserva como discusión en voz alta y alimenta el campo `impacto`
de la línea siguiente. Quedan: pagos anteriores a la emisión (4.3), la señal de fuga obligatoria
(paso 5) y el hallazgo a elección. Tres modeladas y tres escritas.

CIFRAS: las que afirma el texto en Markdown están verificadas contra
`datasets/cobranza_servicios.csv` el 2026-08-03. Si el dataset se regenera hay que volver a
verificarlas. Dentro del CÓDIGO no queda ninguna escrita a mano: las líneas de bitácora modelo
derivan sus magnitudes con f-strings desde el mismo objeto que acaba de imprimir la celda de arriba
(regla de la clase 4, aplicada acá el 2026-08-11). Una cifra fija en un `anotar()` deriva en
silencio si el archivo cambia, y deja el ejemplo modelo contradiciendo su propia evidencia.
...
Uso:  python ediciones/2026/notebooks/_build_03_exploratorio.py
