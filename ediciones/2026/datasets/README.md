...
| `clientes_ventas.parquet` | 3.314 × 9 | Derivado del anterior: agregado **por cliente** con un corte temporal (CC BY 4.0) | Clase 7 |
...
## `clientes_ventas.parquet`

La tabla **por cliente** de la clase 7, derivada de `ventas_online_limpio.parquet`. Es el primer
problema predictivo **bien formulado** del curso: qué va a hacer un cliente **después** de una fecha,
usando solo lo que se sabía **antes** de esa fecha.

**Un registro es un CLIENTE**, no una línea de factura ni una factura. Las clases 4 a 6 usaron el
mismo archivo de origen con las otras dos unidades de observación, y su objetivo —el monto de la propia
factura— era un instrumento de medición, no una decisión de negocio. Acá la unidad cambia y hay
horizonte temporal de verdad.

### El corte, que es lo que ordena todo el archivo

Hay una fecha de corte, **2011-09-01**, y parte el año en dos ventanas:

| Ventana | Período | Para qué sirve |
|:--|:--|:--|
| **Base** | 2010-12-01 a 2011-08-31 | De acá salen TODAS las predictoras. Nueve meses de historia |
| **Futura** | 2011-09-01 a 2011-12-09 | De acá sale SOLO el objetivo. Tres meses de horizonte |

**El sufijo `_base` en el nombre de una columna no es decorativo:** dice que ese dato estaba
disponible en el momento de predecir. Es la lección de la clase 2 —el momento de la predicción— y la
de la clase 6 —de dónde viene cada variable— aplicadas al diseño del archivo, no a una advertencia.
El generador lo **comprueba en las dos direcciones**: reconstruye la tabla con las líneas posteriores
al corte borradas y exige que las predictoras salgan idénticas, y verifica que el objetivo se
reproduzca usando solo la ventana futura.
...
- **Licencia:** Creative Commons Attribution 4.0 International (CC BY 4.0) —
  <https://creativecommons.org/licenses/by/4.0/>
- **Modificación:** obra derivada en **tres pasos**. `_prep_ventas_online.py` renombró las columnas al
  español; `_soluciones/_build_ventas_online_limpio.py` aplicó las siete decisiones de limpieza de la
  clase 4; y [`_prep_clientes_ventas.py`](_prep_clientes_ventas.py) agrega por cliente con el corte
  temporal, quedando 3.314 registros y 9 columnas.

### Diccionario de datos

| Columna | Tipo | Descripción |
|:--|:--|:--|
| `id_cliente` | entero | Identificador del cliente. En el archivo de origen llega como **decimal** —el defecto de tipo que discute la clase 4—; acá se convierte a entero, y es la única transformación de tipo del generador |
| `facturas_base` | entero | Facturas distintas emitidas al cliente en la ventana base |
| `monto_base` | decimal | Monto total facturado en la ventana base, en libras |
| `productos_base` | entero | Códigos de producto distintos que compró en la ventana base |
| `ticket_medio_base` | decimal | `monto_base` dividido por `facturas_base`, en libras |
| `recencia_dias` | entero | Días entre su última compra de la ventana base y el corte |
| `antiguedad_dias` | entero | Días entre su primera compra de la ventana base y el corte |
| `mercado` | categórica nominal | `reino unido` o `exportacion`. Agrupa los 36 países del origen. **`exportacion` es la categoría de referencia** de la clase, la que queda absorbida en el intercepto con la convención k−1 |
| `facturas_futuras` | entero | **Variable objetivo**: facturas distintas emitidas al cliente en la ventana futura |

### Tres cosas que conviene saber antes de usarlo

- **El objetivo es un CONTEO**, entero y no negativo, con **1.367 clientes en cero** —el 41 %—. Por eso
  su mediana es 1 mientras su promedio es 1,63, y su máximo es 95. La familia de modelos que le
  corresponde formalmente —regresión de Poisson— se nombra en la clase 7 y queda fuera de su alcance.
- **El cliente que no vuelve vale cero, no faltante.** Es la única decisión declarada del archivo, y
  es una observación válida: no volvió. Misma decisión que la grilla de empleado-mes de la clase 2. La
  tabla no tiene ningún faltante, porque cada columna se construye.
- **La población NO se filtra por el objetivo.** Un borrador anterior recortaba el 1 % superior del
  gasto futuro para estabilizar los coeficientes: eso es seleccionar por el resultado que no está
  disponible al predecir, o sea la misma falta que la clase 6 enseña a detectar. Se descartó, y el
  generador no lo reintroduce. Tampoco se filtra por país.

### Determinista y falla cerrado

[`_prep_clientes_ventas.py`](_prep_clientes_ventas.py) no usa semillas, ni fechas del sistema, ni red:
el corte está escrito en el código, y correrlo dos veces produce el mismo archivo byte a byte
—verificado—. Comprueba con `assert` cada cifra que el material de la clase afirma y, si alguna no se
cumple, **no escribe el archivo**. Las cifras que publica la clase 7 salen de
[`_build_c07_figuras.py`](../../../assets/ilustraciones/_build_c07_figuras.py), que las imprime al
correr y las protege con sus propias aserciones.
